...
Pow_correc_factor = perceptual_constants.Pow_correc_factor_Hann
...
# Matrices
mat = scipy.io.loadmat('bark_matrix_16k.mat')  # Not nice with absolute path
...
def overlapped_windowing(tensor, window, shift):
    ...
    * General purpose function *
    ...
    overlapped_slices = tf.squeeze(tf.image.extract_patches(tensor[:, :, None, None], sizes=[1, window_size, 1, 1],
...
def squared_magnitude_computation(tensor):
    ...
    """
    spectrum = tf.square(tf.abs(tf.signal.rfft(tensor)))
    return spectrum
...
def magnitude_at_standard_listening_level(spectra):
    ...
    Pow = tf.reduce_mean(input_tensor=tf.multiply(spectra, tf.constant(mask, dtype=tf.float32)))
    return spectra * (10000000.0 / (Pow + 1e-10))
...
def compute_audible_power(bark_spectra, factor=1.0):
    ...
    return tf.reduce_sum(input_tensor=tf.compat.v1.where(cond, bark_spectra, tf.zeros(tf.shape(input=bark_spectra))), axis=1)
...
def bark_gain_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    gain = tf.math.divide(audible_power_ref + 5.0e3, audible_power_deg + 5.0e3)
...
def bark_frequency_equalization(bark_spectra_ref, bark_spectra_deg):
    ...
    ref_thresholded = tf.compat.v1.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               tf.zeros(tf.shape(input=bark_spectra_ref)))
    deg_thresholded = tf.compat.v1.where(tf.greater_equal(bark_spectra_deg, abs_thresh_power * 100.0), bark_spectra_deg,
                               tf.zeros(tf.shape(input=bark_spectra_deg)))

    # Average Pow per bark bin in reference and degraded signals
    avg_pow_per_bark_ref = tf.reduce_sum(input_tensor=tf.compat.v1.where(not_silent, ref_thresholded, tf.zeros(tf.shape(input=bark_spectra_ref))),
                                         axis=0)
    avg_pow_per_bark_deg = tf.reduce_sum(input_tensor=tf.compat.v1.where(not_silent, deg_thresholded, tf.zeros(tf.shape(input=bark_spectra_deg))),
                                         axis=0)
...
def bark_frequency_equalization_on_degraded(bark_spectra_ref, bark_spectra_deg):
    ...
    ref_thresholded = tf.compat.v1.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_ref,
                               tf.zeros(tf.shape(input=bark_spectra_ref)))
    deg_thresholded = tf.compat.v1.where(tf.greater_equal(bark_spectra_ref, abs_thresh_power * 100.0), bark_spectra_deg,
                               tf.zeros(tf.shape(input=bark_spectra_deg)))
...
def loudness_computation(bark_spectra):
    ...
    """
    loudness_dens = Sl * tf.multiply(tf.pow(abs_thresh_power / 0.5, modified_zwicker_power),
                                     tf.pow(0.5 + 0.5 * bark_spectra / abs_thresh_power, modified_zwicker_power) - 1.0)

    cond = tf.less(bark_spectra, abs_thresh_power)
    loudness_dens_limited = tf.compat.v1.where(cond, tf.zeros(tf.shape(input=loudness_dens)), loudness_dens)
...
def compute_distortion_tensors(bark_spectra_ref, bark_spectra_deg):
    ...
    # print('ori', original_loudness)
    # print('dis', distorted_loudness)
    ...
    Asym = tf.pow(tf.math.divide(bark_spectra_deg + 50.0, bark_spectra_ref + 50.0), 1.2)
    cond = tf.less(Asym, 3.0)
    AF = tf.compat.v1.where(cond, tf.zeros(tf.shape(input=Asym)), tf.minimum(Asym, 12.0))
...
def per_frame_distortion(D, DA, total_power_ref):
    ...
    D_frame = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(tf.multiply(D, width_of_band_bark)), axis=1)) * sqrt_total_width
    DA_frame = tf.reduce_sum(input_tensor=tf.multiply(DA, width_of_band_bark), axis=1)
    ...
    wD_frame = tf.minimum(tf.math.divide(D_frame, weights), 45.0)
    wDA_frame = tf.minimum(tf.math.divide(DA_frame, weights), 45.0)
...
def per_frame_PMSQE(ref_spectra, deg_spectra, alpha=0.1):
    ...
    """
    # print()
    # print('reg_spec', ref_spectra)
    # print('deg_spec', deg_spectra)
    if SLL_equalization:
    ...
    # print('ref_base', ref_eq_spectra)
    # print('deg_base', deg_eq_spectra)
    # Bark spectra computation:
    ref_bark_spectra = bark_computation(ref_eq_spectra)
    deg_bark_spectra = bark_computation(deg_eq_spectra)
    # print('ref_bark', ref_bark_spectra)
    # print('deg_bark', deg_bark_spectra)
    ...
    # print('ref_corr', ref_bark_spectra_corrected)
    # print('deg_corr', deg_bark_spectra_corrected)
...
def test_PMSQE(reference_file, distorted_file):
    ...
    signals = tf.compat.v1.placeholder(tf.float32, [2, None])
    ...
    # plt.show()
    # pass
    return np.mean(pmsqe)


if __name__ == "__main__":
    # print(tf.__version__)
    # Tested on tensorflow version 1.10.0
    # test_PMSQE('utils/clean.wav', 'utils/noisy-20.wav')
    # print("Average PMSQE distortion: ", test_PMSQE('../Speech_Enhancement/train/clean/book_00000_chp_0009_reader_06709_18_00.wav', '../Speech_Enhancement/train/noisy/book_00000_chp_0009_reader_06709_18_00_vIYrc23SCf0_10.wav'))
    test = tf.convert_to_tensor(np.zeros((125, 257), dtype=np.float32))
    # print(magnitude_at_standard_listening_level(test))